...
def build(
    ...
    m.fs.system_salt_rejection = Expression(
        expr=1
        - m.fs.product.properties[0].conc_mass_phase_comp["Liq", "TDS"]
    ...
    m.fs.final_permeate_concentration = Expression(
        expr=m.fs.product.flow_mass_phase_comp[0, "Liq", "TDS"]
        / sum(m.fs.product.flow_mass_phase_comp[0, "Liq", j] for j in ["H2O", "TDS"])
    )
...
def set_operating_conditions(m, Cin=None, Qin=None):
    ...
    m.fs.feed.properties[0].conc_mass_phase_comp["Liq", "TDS"] = Cin
    m.fs.feed.properties.calculate_state(
        var_args={
            ("conc_mass_phase_comp", ("Liq", "TDS")): value(
                m.fs.feed.properties[0].conc_mass_phase_comp["Liq", "TDS"]
    ...
    # ---scaling---
    #m.fs.properties.set_default_scaling("flow_mass_phase_comp", 1, index=("Liq", "H2O")) #default WaterTAP scaling
    m.fs.properties.set_default_scaling("flow_mass_phase_comp", 1e-2, index=("Liq", "H2O")) #adjusted scaling
    #m.fs.properties.set_default_scaling("flow_mass_phase_comp", 1e2, index=("Liq", "TDS")) #default WaterTAP scaling
    m.fs.properties.set_default_scaling("flow_mass_phase_comp", 1e-1, index=("Liq", "TDS")) #adjusted scaling
    ...
    print(
        ...
        % (value(m.fs.feed.flow_mass_phase_comp[0, "Liq", "TDS"]) * 1000)
    )
...
def optimize_set_up(
    ...
    if B_case == BCase.single_optimum:
        ...
        m.fs.B_comp_system.set_value(
            m.fs.ROUnits[m.fs.LSRRO_Stages.first()].B_comp[0, "TDS"]
        )
        ...
        if idx > m.fs.Stages.first():
            ...
            if B_case == BCase.single_optimum:
                stage.B_comp_equal = Constraint(
                    expr=stage.B_comp[0, "TDS"] == m.fs.B_comp_system
                )
            if A_case == ACase.single_optimum:
    ...
    if Cbrine is not None:
        ...
        m.fs.ROUnits[m.fs.Stages.last()].feed_side.properties[
            0, 1
        ].conc_mass_phase_comp["Liq", "TDS"].fix(Cbrine)
    ...
    if permeate_quality_limit is not None:
        if isinstance(permeate_quality_limit, (int, float)):
            m.fs.ROUnits[1].mixed_permeate[0].mass_frac_phase_comp["Liq", "TDS"].setub(
                permeate_quality_limit
            )
        else:
...
def display_design(m):
    ...
    for stage in m.fs.Stages:
        ...
        print(
            ...
            % (stage, m.fs.ROUnits[stage].B_comp[0, "TDS"].value * (1000.0 * 3600.0))
        )
...
def display_system(m):
    ...
    brine_flow_mass = sum(
        m.fs.disposal.flow_mass_phase_comp[0, "Liq", j].value for j in ["H2O", "TDS"]
    )
    brine_mass_frac_TDS = (
        m.fs.disposal.flow_mass_phase_comp[0, "Liq", "TDS"].value / brine_flow_mass
    )
    print("Brine: %.3f kg/s, %.0f ppm" % (brine_flow_mass, brine_mass_frac_TDS * 1e6))
